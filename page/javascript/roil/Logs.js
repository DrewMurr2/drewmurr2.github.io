...
var OptionsDropDownHTML = '<a href="#" class="dropdown-toggle" data-toggle="dropdown" role="button" aria-haspopup="true" aria-expanded="false">Options <span class="caret"></span></a><ul class="dropdown-menu"><li class="option" onclick="timeSpanClick()">Time Span<div class="status_indicator"></div></li><li class="option" onclick="liveClick()">Live<div class="status_indicator"></div></li><li class="option" onclick="viewOptionClick()">View<div class="status_indicator"></div></li></ul>'
...
function generateTracksDropDownHTML () {
   ...
   TracksDropDownHTML += 
                            '<li class="track" onclick="HandleTrackClick(\'' + track.Name + '\', ' + (i) + ', this)">' + track.Name + '<div class="status_indicator"></div></li>'  
   i++
        })
...
function loadLogsPageMenu(){
    if(TracksDropDownHTML !== undefined){$('#tracksdropdownmenu').html(TracksDropDownHTML)}
     if(OptionsDropDownHTML !== undefined){$('#optionsdropdownmenu').html(OptionsDropDownHTML)}
}
...
function generate_coordinates(obj){
        ...
        obj.Tracks.forEach(function (track) {
                    var coordinates = combine_x_y(track.Values, obj.RealDates);
            track.colorposition = Math.random() + 0.27
                    track.status = 0
...
function HandleTrackClick(name, index, listElement) { //Build Modal on track click
 ...
 <div id="range_slider3" class="slider"></div>\
    ...
    toggle_activate(status, index);

}


function toggle_activate(oldStatus, index) { // This is the button to turn tracks on and off
    // enable
    if (oldStatus == 0) {
        turnTrackOn(index)
    }
...
function turnTrackOn(i) {
    ...
    chart.options.data[i].dataPoints =  overView.Tracks[i].coordinates;
...
function adjust_chartjsHeight() { // function when window viewport is changed
    ...
    $("#chart_container").css({ 'max-height': container_height + 'px', 'height': container_height + 'px' });
    chart.render();
...
function updateStartTime(){
    ...
    $('#locationLabel').html(chopDate(new Date(startTime)))
      ...
      chart.options.data[overView.Tracks.length].dataPoints =  [   // Y: [Low, High]
				{x: 2, y:[startTime, endtime]},
				{x: 98, y:[startTime, endtime]}
			]
      chart.render()
}
